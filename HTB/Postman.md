...
This webmin vulnerability is a command injection vulnerability in the /package-updates/update.cgi when updating packages on the web interface. Due to improper sanitisation of the request body, we are able to add in a malicious command into the request body and cause the command to be executed
